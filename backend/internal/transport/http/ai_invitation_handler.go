...
func (h *AIInvitationHandler) AnalyzeInvitationIntent(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.Error(w, http.StatusInternalServerError, "Failed to analyze invitation intent", err)
		return
	}
...
func (h *AIInvitationHandler) FindMatchingCharacters(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.Error(w, http.StatusInternalServerError, "Failed to find matching characters", err)
		return
	}
...
func (h *AIInvitationHandler) GenerateInvitationSuggestions(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.Error(w, http.StatusInternalServerError, "Failed to generate invitation suggestions", err)
		return
	}
...
func (h *AIInvitationHandler) ExecuteSmartInvitation(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.Error(w, http.StatusInternalServerError, "Failed to execute smart invitation", err)
		return
	}
...
func (h *AIInvitationHandler) QuickInvite(w http.ResponseWriter, r *http.Request) {
	...
	// 如果需要分析详情，也可以返回
	analysis, _ := h.aiInvitationService.AnalyzeInvitationIntent(r.Context(), req.Message, req.GroupChatID, userID)
